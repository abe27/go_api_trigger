...
func GetCheckStockDetail(c *fiber.Ctx) error {
	...
	sqlFetch := fmt.Sprintf("SELECT c.TAGRP,c.PARTNO,'' partname,c.LOTNO,c.RUNNINGNO,c.STOCKQUANTITY,c.SHELVE,c.PALLETKEY,c.CASEID,CASE WHEN c.CASENO IS NULL THEN 0 ELSE c.CASENO END ,s.STKTAKECHKFLG,c.UPDDTE FROM TXP_CARTONDETAILS c INNER JOIN TXP_STKTAKECARTON s ON c.TAGRP=s.TAGRP AND c.PARTNO=s.PARTNO  AND c.RUNNINGNO=s.RUNNINGNO WHERE c.TAGRP='%s' AND c.PARTNO='%s' ORDER BY c.TAGRP,c.PARTNO,c.LOTNO,c.RUNNINGNO", tagrp, partQuery)
	// fmt.Printf("%s\n", sqlFetch)
	rows, err := db.Query(sqlFetch)
	if err != nil {
		fmt.Println(".....Error processing query")
